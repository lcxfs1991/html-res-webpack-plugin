...
CopyAssetPlugin.prototype.apply = function(compiler) {
    compiler.hooks.afterEmit.tap("CopyAssetPlugin", compilation => {
        ...
        let manifest = path.join(
            config.path.dist,
            "/resource-copy-plugin-3/manifest.json"
        );
        ...
        fs.writeFileSync(
            manifest,
            JSON.stringify(assetObject, null, 4),
            "utf-8"
        );
    });
};
...
module.exports = {
    ...
    entry: {
        "js/index": [
            path.join(config.path.src, "/resource-copy-plugin-3/index")
        ]
    },
    output: {
        ...
        new HtmlResWebpackPlugin({
            ...
            templateContent: function(tpl) {
                ...
                tpl = tpl.replace(regex, function(script, route) {
                    if (
                        !!~script.indexOf("react.js") ||
                        !!~script.indexOf("react-dom.js")
                    ) {
                        return "";
                    }
                    return script;
