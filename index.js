...
function removeAsset(tag) {
  utils.alert(
    tag + " is removed because assets is not found.",
...
function replaceTag(removeUnMatchedAssets, route, tag) {
  return removeUnMatchedAssets && !hasProtocal(route)
    ? removeAsset.bind(this)(tag)
    : tag;
}
...
HtmlResWebpackPlugin.prototype.checkRequiredOptions = function(options) {
  ...
  var injectChunks = _.isArray(options.chunks)
    ? options.chunks
...
HtmlResWebpackPlugin.prototype.apply = function(compiler) {
  ...
  this.options.templateLoaderName = this.getFullTemplatePath(
  ...
  assets.map(asset => {
    ...
    if (chunkName) {
      if (typeof this.stats.assets[chunkName] === "undefined") {
        ...
        this.stats.assets[chunkName] = this.stats.assets[chunkName].concat(
          asset
        );
      }
    }
...
HtmlResWebpackPlugin.prototype.evaluateCompilationResult = function(
  compilation,
  source
) {
  if (!source) {
  ...
  var template = this.options.templateLoaderName
    .replace(/^.+!/, "")
    .replace(/\?.+$/, "");
...
HtmlResWebpackPlugin.prototype.buildStats = function(compilation) {
  ...
  let optionChunks = this.options.chunks,
    injectChunks = _.isArray(optionChunks)
      ? optionChunks
      : Object.keys(optionChunks);
...
// 匹配资源
HtmlResWebpackPlugin.prototype.checkResource = function(
  htmlContent,
  publicPath,
  compilation
) {
...
HtmlResWebpackPlugin.prototype.md5HtmlRes = function(
  routeStr,
  reg,
  publicPath
) {
...
HtmlResWebpackPlugin.prototype.inlineHtmlRes = function(
  routeStr,
  reg,
  publicPath,
  compilation
...
HtmlResWebpackPlugin.prototype.injectAssets = function(compilation) {
  ...
  injectChunks.map(chunkKey => {
    ...
    this.stats.assets[chunkKey].map(file => {
      ...
      if (!file) {
        utils.alert(
          `${chunkKey} is not found but defined in option`,
          this.options.logLevel
        );
        return;
      }
      let fileType = utils.getFileType(file),
        isExternal =
          (optionChunks[chunkKey] && optionChunks[chunkKey].external) || false;

      switch (fileType) {
        case "js": {
          let jsInline = false;
          if (!_.isArray(optionChunks)) {
            jsInline = this.inlineRes(
              compilation,
              optionChunks[chunkKey],
              file
            );
          }
